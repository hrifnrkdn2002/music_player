...
import 'package:music_player/service/youtube_service.dart';

import 'database_helper.dart';

// 1. GetIt 인스턴스 생성
// main.dart로드 할 때 import get_it.dart 시 실행
final GetIt locator = GetIt.instance;

// 2. 등록을 위한 함수 정의
Future<void> setupLocator() async {
  // 여기에 등록할 클래스들을 정의합니다.
  // DatabaseHelper 객체를 하나 생성
  // database 메서드를 실행해서 static Database 객체를 가져옴 db에 저장
  final helper = DatabaseHelper();
  final db = await helper.database;

  //데이터베이스 영역 싱글톤으로 등록
  locator.registerSingleton<Database>(db); //sqlite가 제공하는 Database타입으로 db를 싱글톤으로 등록
  ...
  //오디오 영역 싱글톤으로 등록
  //YoutubeService는 PlaybackHandler가 유튜브 소스를 만들 때 필요하므로 먼저 등록
  locator.registerSingleton<YoutubeServiceInterface>(YoutubeService());
  ...
  try {
    handler = await AudioService.init(
      builder: () => PlaybackHandler(locator<YoutubeServiceInterface>()),
    ...
    handler = PlaybackHandler(locator<YoutubeServiceInterface>());
  }
