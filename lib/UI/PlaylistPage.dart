...
class _PlaylistPageState extends State<PlaylistPage> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      context.read<DB_Provider>().loadPlaylists();
    });
  }

  @override
  Widget build(BuildContext context) {
    // 💡 테마 및 Provider 데이터 가져오기
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    trailing: PopupMenuButton<String>(
                      ...
                      onSelected: (value) {
                        if (value == 'edit') {
                          _showEditPlaylistDialog(context, playlistId, playlistName, isDark);
                        } else if (value == 'delete') {
                          _showDeleteConfirmDialog(context, playlistId, playlistName, isDark);
                        }
                      },
                      itemBuilder: (context) => [
                        PopupMenuItem(
  ...
  void _showCreatePlaylistDialog(BuildContext context, bool isDark) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('새 플레이리스트', style: TextStyle(color: isDark ? Colors.white : Colors.black)),
        content: TextField(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (name.isNotEmpty) {
                context.read<DB_Provider>().createPlaylist(name);
                Navigator.pop(context);
              }
            },
            child: Text('생성', style: TextStyle(color: isDark ? Colors.purpleAccent : Colors.purple)),
          ),
        ],
      ),
    );
  ...
  void _showEditPlaylistDialog(BuildContext context, int id, String currentName, bool isDark) {
    final controller = TextEditingController(text: currentName);
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: isDark ? const Color(0xFF1C1C1C) : Colors.white,
        title: Text('이름 수정', style: TextStyle(color: isDark ? Colors.white : Colors.black)),
        content: TextField(
          controller: controller,
          style: TextStyle(color: isDark ? Colors.white : Colors.black),
          decoration: InputDecoration(
          ...
          TextButton(
            onPressed: () {
              ...
              if (newName.isNotEmpty && newName != currentName) {
                context.read<DB_Provider>().updatePlaylistName(id, newName);
                Navigator.pop(context);
              }
            },
            child: Text('수정', style: TextStyle(color: isDark ? Colors.purpleAccent : Colors.purple)),
          ),
        ],
      ),
    );
  ...
  void _showDeleteConfirmDialog(BuildContext context, int id, String name, bool isDark) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: isDark ? const Color(0xFF1C1C1C) : Colors.white,
        title: Text('플레이리스트 삭제', style: TextStyle(color: isDark ? Colors.white : Colors.black)),
        content: Text(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              context.read<DB_Provider>().deletePlaylist(id);
              Navigator.pop(context);
            },
